...
`baselines/IPPO/ippo_overcooked_v3.py`와 `fcp_overcooked_v3.py`는 matmul 정밀도를 지정하지
않아 jax 기본값(`jax_default_matmul_precision=None`)을 쓴다. 그러면 Ampere/Ada(A5000, L4)는
TF32(가수 10비트)를, Turing(2080 Ti)은 FP32(가수 24비트)를 쓴다. 반올림 순서 차이가 아니라
정밀도 자체가 다르다.

알고리즘과 수식은 동일하므로 실험의 의미는 바뀌지 않는다. 완료된 multimap 72런에서 두 GPU
간 체계적 차이를 찾아보면 12개 그룹(알고리즘 × arm × 맵)의 평균 차가 +11.7 ± 26.8,
t = 1.52로 시드 분산에 묻힌다. 다만 그룹마다 시드 구성이 달라 통제된 비교가 아니므로 "효과가
0"이라는 근거는 못 된다. 카오스적 학습이라 개별 런 수준에서 머신 효과를 분리하는 것은
원리적으로 불가능하고, 분포 수준에서 검출하려면 머신당 수십 런이 필요하다.

`JAX_DEFAULT_MATMUL_PRECISION=highest`로 전 머신을 FP32로 통일할 수 있지만, 이미 끝난 런들이
기본값으로 돌았으므로 **실험 도중에 바꾸면 안 된다**. 두 정밀도로 쪼개진 결과가 된다.

실무적으로는 arm 쌍(cdon/cdoff)을 같은 머신에 두면 이 차이가 arm 대조에 실리지 않는다.
비용이 0일 때만 지키면 된다.
