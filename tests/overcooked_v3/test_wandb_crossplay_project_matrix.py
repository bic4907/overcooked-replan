...
def test_wandb_run_name_starts_with_xp():
    ...
    assert evaluation_run_name(settings) == "xp-ippo+fcp-split_0"
...
def test_reproducibility_bundle_saves_command_config_and_executable_sources(
    ...
    write_reproducibility_bundle(tmp_path, settings, "xp-ippo-split_0", artifact_dir)
    ...
    assert config["wandb_run_name"] == "xp-ippo-split_0"
...
def test_pair_results_csv_records_the_map_in_the_first_column(tmp_path):
    ...
    _write_records_csv(
        csv_path,
        [{"layout": "split_0", "pair_type": "XP", "mean_return": 3.0}],
    )
    ...
    with csv_path.open(encoding="utf-8", newline="") as stream:
        ...
        assert list(reader) == [
            {"map": "split_0", "pair_type": "XP", "mean_return": "3.0"}
        ]
...
def test_run_filters_push_layout_seed_and_state_to_wandb():
    assert build_run_filters(["split_0"], [1, 2], "finished") == {
        "config.ENV_KWARGS.layout": {"$in": ["split_0"]},
        "config.SEED": {"$in": [1, 2]},
        "state": "finished",
    }
...
def test_discovery_keeps_latest_run_per_algorithm_layout_and_seed():
    ...
    wrong_layout = fake_run("wrong", algorithm="IPPO", layout="outage_0")
