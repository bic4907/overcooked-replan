...
def test_each_role_scenario_family_has_ten_unique_layouts():
    ...
    for family, names in ROLE_SCENARIO_LAYOUTS.items():
        assert names == tuple(f"{family}_{variant}" for variant in range(10))
        signatures = {
            tuple(
...
def test_all_role_scenario_variants_are_resettable(layout_name):
    ...
    assert obs["agent_0"].shape == (7, 11, 33)
...
@pytest.mark.parametrize("variant", range(10))
def test_each_split_variant_preserves_anti_coordination_dynamics(variant):
    ...
    assert (door_x - 1, door_y) in _reachable_floor(closed_phase, left_start)
    assert (door_x + 1, door_y) in _reachable_floor(closed_phase, right_start)

    onion = StaticObject.ingredient_pile(0)
    for phase in (open_phase, closed_phase):
        assert np.sum(phase[:, :5] == onion) == 1
        assert np.sum(phase[:, 6:] == onion) == 0
        assert np.sum(phase[:, :5] == StaticObject.POT) == 2
        assert np.sum(phase[:, 6:] == StaticObject.POT) == 0
        assert np.sum(phase[:, :5] == StaticObject.PLATE_PILE) == 0
        assert np.sum(phase[:, 6:] == StaticObject.PLATE_PILE) == 1
        assert np.sum(phase[:, :5] == StaticObject.GOAL) == 0
        assert np.sum(phase[:, 6:] == StaticObject.GOAL) == 1

    left_reachable = _reachable_floor(closed_phase, left_start)
    right_reachable = _reachable_floor(closed_phase, right_start)
    for object_type in (onion, StaticObject.POT):
        for y, x in np.argwhere(closed_phase == object_type):
            assert _can_interact_from(closed_phase, (x, y), left_reachable)
    for object_type in (StaticObject.PLATE_PILE, StaticObject.GOAL):
        for y, x in np.argwhere(closed_phase == object_type):
            assert _can_interact_from(closed_phase, (x, y), right_reachable)
    signal_y, signal_x = np.argwhere(closed_phase == StaticObject.RECIPE_INDICATOR)[0]
    assert signal_x == 5
    assert (signal_x - 1, signal_y) in left_reachable
    assert (signal_x + 1, signal_y) in right_reachable


@pytest.mark.parametrize("variant", range(10))
def test_each_outage_variant_preserves_division_of_labor_dynamics(variant):
    layout = dynamic_layouts[f"outagenosig_{variant}"]
    assert tuple(phase.steps for phase in layout.phases) == (100, 100)
    normal_phase = layout.phases[0].layout.static_objects
    outage_phase = layout.phases[1].layout.static_objects
    left_start, right_start = layout.phases[0].agent_positions

    for phase in (normal_phase, outage_phase):
        assert right_start not in _reachable_floor(phase, left_start)
    differences = np.argwhere(normal_phase != outage_phase)
    assert differences.shape == (1, 2)
    outage_y, outage_x = differences[0]
    onion = StaticObject.ingredient_pile(0)
    assert outage_x > 5
    ...
    for object_type in (
        ...
        assert np.sum(normal_phase[:, :5] == object_type) == 1
        assert np.sum(normal_phase[:, 6:] == object_type) == 1
    assert np.sum(outage_phase[:, :5] == onion) == 1
    assert np.sum(outage_phase[:, 6:] == onion) == 0
    for object_type in (StaticObject.POT, StaticObject.PLATE_PILE, StaticObject.GOAL):
        assert np.sum(outage_phase[:, :5] == object_type) == 1
        assert np.sum(outage_phase[:, 6:] == object_type) == 1

    left_reachable = _reachable_floor(outage_phase, left_start)
    right_reachable = _reachable_floor(outage_phase, right_start)
    for phase, reachable, column_slice in (
        (outage_phase, left_reachable, slice(None, 5)),
        (outage_phase, right_reachable, slice(6, None)),
    ):
        for object_type in (
            StaticObject.POT,
            StaticObject.PLATE_PILE,
            ...
            for y, relative_x in np.argwhere(phase[:, column_slice] == object_type):
                x = relative_x + (6 if column_slice.start == 6 else 0)
                assert _can_interact_from(phase, (x, y), reachable)
    for y, x in np.argwhere(outage_phase[:, :5] == onion):
        assert _can_interact_from(outage_phase, (x, y), left_reachable)

    signal_y, signal_x = np.argwhere(outage_phase == StaticObject.RECIPE_INDICATOR)[0]
    assert signal_x == 5
    assert (signal_x - 1, signal_y) in left_reachable
    assert (signal_x + 1, signal_y) in right_reachable
    handoff_rows = [
        ...
        if outage_phase[y, 5] == StaticObject.WALL
        and (4, y) in left_reachable
        and (6, y) in right_reachable
    ]
    assert handoff_rows
