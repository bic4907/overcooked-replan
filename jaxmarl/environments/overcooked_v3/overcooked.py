...
class State(BaseState):
    """OvercookedV3Base environment state.
    ...
    layout_index: jax.Array = struct.field(
...
class OvercookedV3Base(MultiAgentEnv):
    ...
    def __init__(
        ...
        Args:
            layout (Layout): The layout configuration for the environment, defaulting to "cramped_room". Either a Layout object or a string key to look up a Layout in overcooked_v3_base_layouts.
        ...
        if isinstance(layout, str):
            if layout not in overcooked_v3_base_layouts:
                raise ValueError(
                    f"Invalid layout: {layout}, allowed layouts: {overcooked_v3_base_layouts.keys()}"
                )
            layout = overcooked_v3_base_layouts[layout]
    ...
    def _randomize_agent_positions(self, state: State, key: PRNGKeyArray) -> State:
        ...
        num_agents = self.num_agents
        agents = state.agents
        enclosed_spaces = self._get_enclosed_spaces(state)

        def _select_agent_position(taken_mask, x):
            pos, key = x

            allowed_positions = (
                enclosed_spaces == enclosed_spaces[pos.y, pos.x]
            ) & ~taken_mask
    ...
    def get_obs_featurized(self, state: State) -> BatchedObservations:
        ...
        def _player_features(agent):
            ...
            reachable_area = enclosed_spaces == enclosed_spaces[pos.y, pos.x]
            ...
            def _closest_features(
                ...
                obj_pos, is_valid = self._get_closest_target_pos(
                    state, mask, pos, direction
                )
            ...
            for i in range(num_pots):
                ...
                pot_pos, is_valid = self._get_closest_target_pos(
                    state, pot_mask, pos, direction
                )
    ...
    def _get_move_area(self, state: State) -> jax.Array:
        return state.grid[:, :, 0] == StaticObject.EMPTY
    ...
    def _get_closest_target_pos(self, state, targets, pos, direction):
        return self.path_planer.get_closest_target_pos(targets, pos, direction)
    ...
    def step_agents(
        ...
        def _move_wrapper(agent, action):
            ...
            def _move(agent, dir):
                ...
                new_pos = tree_select(
                    self._get_move_area(state)[new_pos.y, new_pos.x], new_pos, pos
                )
