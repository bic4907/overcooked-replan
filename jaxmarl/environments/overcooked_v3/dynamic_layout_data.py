...
def _inversion_distance_switch_detour_spec():
    """Give both roles a remote station, with unequal routes to the pots.

    Each work bay has an upper and lower resource endpoint behind a short
    partition.  The upper endpoint is seven walking steps from a pot access
    cell and the lower endpoint four.  Their onion/serving identities swap
    every 75 steps, making early movement toward the next role useful.
    """
    return {
        "width": 13,
        "height": 6,
