#!/usr/bin/env python3
"""Run both-warning FCP on the 13x6 relocating-stations distance_0 map."""
...
LAYOUT_REVISION = "distance-inversion-relocated-13x6-v5"
GPUS = ["6", "7"]
LAYOUTS = ["distance_0"]
OBSERVERS = ["both"]
SEEDS = list(range(6))
PILOT_CELL = ("distance_0", "both")
POP_PROJECT = "overcooked-v3-fcp-distance0-inversion-relocated-0920_population"
TRAIN_PROJECT = "overcooked-v3-fcp-distance0-inversion-relocated-0920_train"
EVAL_PROJECT = "overcooked-v3-fcp-distance0-inversion-relocated-0920_eval"
...
def stable_id(stage, layout, observer, seed=None):
    raw = f"fcp-distance0-inversion-relocated-0920|{stage}|{layout}|{observer}|{seed}".encode()
...
def command_for(job, gpu):
    ...
    seed = job["seed"]
    common_tags = (
        f"[0920,75-step,distance0-inversion-relocated,13x6,FCP,default-warning,"
    ...
    return [
        ...
        "--run-label", f"FCP-greedy-distance0-inversion-relocated-0920-{layout}-{label}",
    ]
...
def archive_population(state):
    ...
    run = wandb.init(
        ...
        name="population-archive-fcp-distance0-inversion-relocated-0920",
        job_type="population-archive",
        tags=["0920", "75-step", "distance0-inversion-relocated", "13x6",
    ...
    try:
        ...
        artifact = wandb.Artifact(
            "fcp-distance0-inversion-relocated-population-0920", type="fcp-population",
    ...
    state["population_archive"] = {
        "run_id": "popd0h20", "artifact": "fcp-distance0-inversion-relocated-population-0920",
