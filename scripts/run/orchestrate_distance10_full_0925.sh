#!/usr/bin/env bash
# Ten-seed confirmation in separate W&B projects; resumes only missing stages.
set -euo pipefail
...
export CAMPAIGN="distance10-priority-0925-${KIND}-full10"

TRAIN_DONE="$ROOT/orchestrator_distance10_${KIND}_full10_train.done"
EVAL_DONE="$ROOT/orchestrator_distance10_${KIND}_full10_eval.done"
if [[ ! -f "$TRAIN_DONE" ]]; then
    bash "$ROOT/scripts/run/run_topology_inversion.sh" main train \
        > "$ROOT/orchestrator_distance10_${KIND}_full10_train.log" 2>&1
    touch "$TRAIN_DONE"
fi
if [[ ! -f "$EVAL_DONE" ]]; then
    bash "$ROOT/scripts/run/run_topology_inversion.sh" main eval \
        > "$ROOT/orchestrator_distance10_${KIND}_full10_eval.log" 2>&1
    touch "$EVAL_DONE"
fi
